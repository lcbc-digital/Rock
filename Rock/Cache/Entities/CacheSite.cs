...
namespace Rock.Cache
{
    ...
    [Serializable]
    [DataContract]
    public class CacheSite : ModelCache<CacheSite, Site>
    {
        ...
        /// </summary>
        public CachePage DefaultPage
        {
            get
            {
                if ( DefaultPageId.HasValue && DefaultPageId.Value != 0 )
                {
                    return CachePage.Get( DefaultPageId.Value );
                }
        ...
        /// <returns></returns>
        public static CacheSite GetSiteByDomain( string host )
        {
            int? siteId;
